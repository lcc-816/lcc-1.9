...
static void opt(arg) char *arg; {
	...
	case 'W':	/* -Wxarg */
		if (arg[2] && arg[3])
			switch (arg[2]) {
			case 'o': if (option(&arg[3])) return;
			case 'p': plist = append(&arg[3], plist); return;
			case 'f':
				...
				break; /* and fall thru */
			case 'a': alist = append(&arg[3], alist); return;
			case 'l': llist[0] = append(&arg[3], llist[0]); return;
			}
